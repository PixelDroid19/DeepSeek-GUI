...
export function buildModelCompactionPrompt(input: {
  ...
  return [
    ...
    '',
    'After the prose summary, append a fenced JSON code block (```json ... ```) with exactly this shape:',
    '{ "decisions": string[], "filesTouched": string[], "errorsResolved": string[], "pending": string[] }',
    'Each array lists short plain-text entries extracted from the history; use empty arrays when nothing applies.',
    '',
